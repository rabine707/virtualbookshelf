...
function applyVerifiedCover(candidate: Candidate) {
  ...
  const next = books.map((book) => {
    ...
    const cover = { url: candidate.spineImage, source: candidate.source === "web" ? "Community verified web cover" : "Community verified upload" };
...
function rejectCover(candidate: Candidate) {
  const key = identity(candidate.title, candidate.author);
  const next = readLibrary().map((book) => identity(book.title || "", book.author || "") !== key ? book : ({ ...book, coverFeedback: { ...book.coverFeedback, rejected: Array.from(new Set([...(book.coverFeedback?.rejected || []), candidate.spineImage])) } }));
  window.localStorage.setItem(LIBRARY_KEY, JSON.stringify(next));
}
...
export default function SpineCommunityEnricher() {
  ...
  useEffect(() => {
    const mountModalTools = () => {
      ...
      const upload = document.createElement("button"); upload.type = "button"; upload.className = "primary spine-upload-button"; upload.textContent = "📷 Upload candidate";
      const input = document.createElement("input"); input.type = "file"; input.accept = "image/*"; input.hidden = true; upload.onclick = () => input.click();
      input.onchange = async () => { const file = input.files?.[0]; if (!file) return; try { const image = await fileToDataUrl(file); writeCandidates([...readCandidates(), { id: `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, title: book.title, author: book.author, coverUrl: book.coverUrl || libraryCover(book.title, book.author), spineImage: image, source: "upload", createdAt: Date.now() }]); upload.textContent = "✓ Added"; setRevision((v) => v + 1); } catch { upload.textContent = "Upload failed"; } input.value = ""; };
  ...
  const queue = useMemo(() => { void revision; const votes = readVotes(); return readCandidates().filter((candidate) => !votes[candidate.id]); }, [revision, reviewOpen]);
  ...
  async function fillQueue() {
    ...
    setLoading(true);
    try {
      const existing = readCandidates(); const votes = readVotes(); const existingUrls = new Set(existing.map((item) => `${identity(item.title, item.author)}::${item.spineImage}`));
      const books = readLibrary().filter((book) => book.title).filter((book) => {
        const key = identity(book.title || "", book.author || "");
        return !existing.some((item) => identity(item.title, item.author) === key && !votes[item.id]);
      }).slice(0, 2);
      ...
      for (const book of books) {
        ...
        for (const result of (data.results || []).slice(0, 3)) {
          ...
          existingUrls.add(fingerprint); additions.push({ id: `web-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, title: book.title || "", author: book.author || "", coverUrl: book.preferredCover?.url, spineImage: image, source: "web", createdAt: Date.now() });
        }
      }
  ...
  return <>
    ...
    {reviewOpen && createPortal(<div className="spine-review-backdrop" onClick={() => setReviewOpen(false)}><section className="spine-review-sheet" role="dialog" aria-modal="true" aria-label="Help verify book artwork" onClick={(event) => event.stopPropagation()}>
      <header><div><small>Quick community check</small><h2>Help the Shelf</h2><p>Is this the right artwork for this book? One tap and you’re onto the next.</p></div><button type="button" onClick={() => setReviewOpen(false)} aria-label="Close">×</button></header>
      {current ? <>
        <div className="spine-review-book">{current.coverUrl && <img src={current.coverUrl} alt="Current cover" />}<div><strong>{current.title}</strong><span>{current.author}</span><small>{current.source === "web" ? "Artwork found automatically" : "Community candidate"}</small></div></div>
        <div className={`spine-swipe-card ${dragX > 50 ? "match" : dragX < -50 ? "wrong" : ""}`} style={{ transform: `translateX(${dragX}px) rotate(${dragX / 24}deg)` }} onPointerDown={(event) => { startX.current = event.clientX; event.currentTarget.setPointerCapture(event.pointerId); }} onPointerMove={(event) => { if (startX.current !== null) setDragX(Math.max(-150, Math.min(150, event.clientX - startX.current))); }} onPointerUp={finishDrag} onPointerCancel={finishDrag}><img src={current.spineImage} alt={`Candidate artwork for ${current.title}`} /><span className="swipe-wrong">WRONG</span><span className="swipe-match">MATCH</span></div>
        ...
        <footer><span>{queue.length} ready to check</span><span>★ {points} verified</span></footer>
      </> : <div className="spine-review-empty"><span>{loading ? "…" : "✓"}</span><h3>{loading ? "Finding artwork…" : "You’re caught up"}</h3><p>{loading ? "We’re loading likely matches so they’re ready for you to check." : "More likely matches will appear here automatically."}</p>{!loading && <button type="button" className="primary" onClick={() => void fillQueue()}>Find more to check</button>}</div>}
