...
function generatedSpineUrl(coverUrl: string) {
  return `/api/spine?cover=${encodeURIComponent(coverUrl)}`;
}
...
function buildSpine(button: HTMLButtonElement, sourceImage: HTMLImageElement) {
  ...
  // This image is a separate 4:17 spine asset generated by /api/spine. The
  // original cover remains untouched for the detail modal and as a fallback.
  ...
  art.addEventListener("error", () => {
    // Never leave a blank book if a remote cover host rejects server fetching.
    // Falling back to the approved cover keeps the shelf usable.
    art.src = src;
