...
type PublicProfile = { username?: string; display_name?: string; avatar_url?: string; bio?: string; trusted_curator?: boolean; is_demo?: boolean };
...
export default function PublicShelfPage() {
  ...
  useEffect(() => {
    ...
    if (requestedId) setSelectedBook(books.find((book) => book.id === requestedId) || null);
  }, [books]);

  useEffect(() => {
    if (!selectedBook) return;
    const onKeyDown = (event: KeyboardEvent) => { if (event.key === "Escape") closeBook(); };
    document.addEventListener("keydown", onKeyDown);
    return () => document.removeEventListener("keydown", onKeyDown);
  }, [selectedBook]);

  function openBook(book: PublicBook) {
  ...
  async function copyBookLink() {
    try { await navigator.clipboard.writeText(window.location.href); setBookLinkCopied(true); window.setTimeout(() => setBookLinkCopied(false), 1400); } catch { /* optional */ }
  }
  ...
  return <main className="public-shelf-page">
    <style>{`
      ...
      .public-book-backdrop{position:fixed;inset:0;z-index:1100;display:grid;place-items:center;padding:18px;background:rgba(5,4,3,.76);backdrop-filter:blur(10px)}.public-book-dialog{position:relative;width:min(560px,100%);max-height:min(84vh,720px);overflow:auto;display:grid;grid-template-columns:130px minmax(0,1fr);gap:20px;padding:24px;border:1px solid rgba(255,255,255,.12);border-radius:24px;background:#17120f;box-shadow:0 28px 80px rgba(0,0,0,.58)}.public-book-dialog img,.public-book-cover-fallback{width:130px;aspect-ratio:2/3;display:grid;place-items:center;border-radius:10px;object-fit:cover;background:var(--book-color,#5d493a);box-shadow:0 12px 28px rgba(0,0,0,.35);font:700 34px Georgia,serif}.public-book-dialog-copy{align-self:center;min-width:0}.public-book-dialog-copy>small{color:var(--sof-ui-accent,#d6b47c);font:800 9px Arial,sans-serif;letter-spacing:.12em}.public-book-dialog h2{margin:7px 42px 4px 0;font-size:clamp(25px,6vw,38px);line-height:1}.public-book-dialog p{margin:0;color:var(--sof-ui-muted,#aaa)}.public-book-dialog-meta{display:flex;flex-wrap:wrap;gap:6px;margin:15px 0}.public-book-dialog-meta span{padding:6px 8px;border-radius:999px;background:rgba(255,255,255,.06);font:700 10px Arial,sans-serif}.public-book-dialog-share{min-height:42px;padding:0 13px;border:1px solid var(--sof-ui-accent,#d6b47c);border-radius:11px;background:transparent;color:inherit;font-weight:800}.public-book-dialog-close{position:absolute;top:14px;right:14px;width:40px;height:40px;border:1px solid rgba(255,255,255,.12);border-radius:50%;background:rgba(255,255,255,.06);color:inherit;font-size:22px}@media(max-width:520px){.public-book-dialog{grid-template-columns:92px minmax(0,1fr);gap:15px;padding:18px}.public-book-dialog img,.public-book-cover-fallback{width:92px}.public-book-dialog h2{font-size:25px}}
      .public-book-dialog{color:#f2eadf}.public-book-dialog h2,.public-book-dialog button{color:inherit}.public-book-dialog p{color:rgba(242,234,223,.68)}.public-shelf-row{content-visibility:auto;contain-intrinsic-size:auto 238px}
    ...
    {loading ? <div className="public-shelf-state">Loading @{username}’s shelf…</div> : error ? <div className="public-shelf-state"><div><h1>This shelf isn’t public</h1><p>{error}</p><Link href="/readers">Discover public readers</Link></div></div> : data ? <>
      ...
      <section className="public-bookcase" aria-label={`${name}'s bookshelf`}>
        {books.length ? Array.from({length:Math.ceil(books.length/8)},(_,row) => <div className="public-shelf-row" key={row}>{books.slice(row*8,row*8+8).map((book,index) => <article className="public-shared-spine" key={book.id}><MobileBookSpine book={book} index={(row * 8) + index} onSelect={openBook} externalSpineUrl={book.demoSpineUrl || publicSpineUrl(book.spineStoragePath)} /></article>)}</div>) : <div className="public-shelf-empty">No books on this public shelf yet.</div>}
      </section>
      {selectedBook ? <div className="public-book-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) closeBook(); }}><article className="public-book-dialog" role="dialog" aria-modal="true" aria-label={selectedBook.title}><button className="public-book-dialog-close" type="button" aria-label="Close book" onClick={closeBook}>×</button>{selectedBook.preferredCover?.url ? <img src={selectedBook.preferredCover.url} alt={`Cover of ${selectedBook.title}`} /> : <div className="public-book-cover-fallback" style={{ "--book-color": selectedBook.color } as React.CSSProperties}>{selectedBook.title.slice(0,1)}</div>}<div className="public-book-dialog-copy"><small>FROM {name.toUpperCase()}’S SHELF</small><h2>{selectedBook.title}</h2><p>by {selectedBook.author}</p><div className="public-book-dialog-meta">{selectedBook.rating ? <span>{"★".repeat(Math.min(5,selectedBook.rating))}</span> : null}{selectedBook.shelf ? <span>{selectedBook.shelf.replace(/-/g," ")}</span> : null}{selectedBook.year ? <span>{selectedBook.year}</span> : null}</div><button className="public-book-dialog-share" type="button" onClick={() => void copyBookLink()}>{bookLinkCopied ? "Book link copied ✓" : "Share this book"}</button></div></article></div> : null}
    </> : null}
  </main>;
}
