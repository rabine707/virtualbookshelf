...
describe("curator spine deletion guard", () => {
  it.each(["clothbound", "dust-jacket", "special-edition", "left", "center", "right", "gemini-image", null])("allows curator deletion regardless of model %s", (model) => expect(isDeletableCatalogSpine({ ...base, model })).toBe(true));
