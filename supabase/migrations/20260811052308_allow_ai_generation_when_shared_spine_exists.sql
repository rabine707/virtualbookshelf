create or replace function public.consume_spine_generation_attempt(
  ...
  p_title text,
  p_author text,
  p_isbn text default null::text,
  p_asin text default null::text,
...
begin
  ...
  if current_attempts >= p_limit then
    return query select false, current_attempts, 0, shared_path;
    return;
  end if;

  update public.spine_generation_usage
  set attempts = spine_generation_usage.attempts + 1,
      title = coalesce(nullif(p_title,''), spine_generation_usage.title),
      author = coalesce(nullif(p_author,''), spine_generation_usage.author),
      isbn = coalesce(p_isbn, spine_generation_usage.isbn),
      asin = coalesce(p_asin, spine_generation_usage.asin),
      last_generated_at = now(),
      updated_at = now()
  where user_id = uid and book_key = p_book_key
  returning spine_generation_usage.attempts into current_attempts;

  return query select true, current_attempts, greatest(p_limit - current_attempts, 0), shared_path;
end;
$function$;
