-- Connect a reader's confirmed cover choices to the shared community catalog.
-- Trusted curator membership is profile data and is intentionally not hard-coded in this migration.
alter table public.profiles
  add column if not exists trusted_curator boolean not null default false;
...
begin
  ...
  if cid is null then
    ...
    set book_id = case
          when is_trusted then bid
          when c.book_id is null then bid
          else c.book_id
        end,
...
create or replace function public.get_approved_cover_candidates(
  p_title text default null,
  p_author text default null,
  p_isbn text default null,
  p_asin text default null
)
returns table(image_url text, source text, confidence integer, correct_votes integer)
language sql
stable
security definer
set search_path = public
as $$
  with wanted as (
    select
      public.normalize_book_answer(coalesce(p_title, '')) as nt,
      public.normalize_book_answer(coalesce(p_author, '')) as na,
      case
        when upper(regexp_replace(coalesce(p_isbn, ''), '[^0-9Xx]', '', 'g')) ~ '^(?:[0-9]{13}|[0-9]{9}[0-9X])$'
          then upper(regexp_replace(coalesce(p_isbn, ''), '[^0-9Xx]', '', 'g'))
        else null
      end as isbn,
      case
        when upper(regexp_replace(coalesce(p_asin, ''), '[^A-Za-z0-9]', '', 'g')) ~ '^[A-Z0-9]{10}$'
          then upper(regexp_replace(coalesce(p_asin, ''), '[^A-Za-z0-9]', '', 'g'))
        else null
      end as asin
  )
  select
  ...
  where c.status = 'approved'
    and (
      (w.isbn is not null and b.isbn = w.isbn)
      or (w.asin is not null and b.asin = w.asin)
      or (
        char_length(w.nt) > 0 and char_length(w.na) > 0
        and b.normalized_title = w.nt
        and b.normalized_author = w.na
      )
    )
  order by
    case
      when w.isbn is not null and b.isbn = w.isbn then 3
      when w.asin is not null and b.asin = w.asin then 2
      else 1
    end desc,
    c.confidence desc,
...
grant execute on function public.get_approved_cover_candidates(text,text,text,text) to anon, authenticated;
