  ...
  select jsonb_build_object(
    'books', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'coverFeedback',ub.cover_feedback,'savedCovers',ub.saved_covers,'favorite',ub.favorite,
        'selectedSpineId',coalesce(ub.selected_spine_id,s.id),'spineStoragePath',s.storage_path,'spineProvider',s.provider,'spineModel',s.model,'updatedAt',ub.updated_at
      ) order by ub.created_at,b.title)
      from public.user_books ub
      join public.books b on b.id=ub.book_id
      left join lateral (
        select sp.id,sp.storage_path,sp.provider,sp.model
